...
class docs_location {
   ...
   --------------------------------------------------------------- */
	...
	public function get_sections_from_location() {
		// Rajouté car besoin dans la localisation d'exemplaire
		$query = "SELECT num_section FROM docsloc_section where num_location='".$this->id. "'";
		$result = pmb_mysql_query($query);
		$section_ids = array();
		while ($r = pmb_mysql_fetch_assoc($result)) {
			$section_ids[]= $r['num_section'];
		}
		return $section_ids;
	}
